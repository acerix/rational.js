...
rat.fromDecimal_copy=function(a,b){b=parseFloat(b);if(0===b)return rat.copy(a,rat.ZERO);if(1===b)return rat.copy(a,rat.ONE);if(Infinity===b)return rat.copy(a,rat.INFINITY);if(isNaN(b))return rat.copy(a,rat.INFINULL);if(0===b%1)return rat.fromInteger_copy(a,b);if(Math.abs(1/b%1)<rat.EPSILON)return rat.fromIntegerInverse_copy(a,Math.round(1/b));a[0]=1;a[1]=1;for(var c=[1,0,0,1],e=b,d,g,f=NaN,h=rat.MAX_LOOPS;h--;){d=Math.floor(e);a[0]=d*c[0]+c[2];a[1]=d*c[1]+c[3];g=rat.toDecimal(a);if(g===b||g===f)return a;
f=g;e=1/(e-d);c[2]=c[0];c[3]=c[1];c[0]=a[0];c[1]=a[1]}limit||console.log("rat.fromDecimal("+b+") reached limit at "+rat.str(a));return a};rat.fromRandom=function(a){a[0]=0xfffffffffffff*Math.random()<<0;a[1]=Math.abs(0xfffffffffffff*Math.random()<<0);return rat.normalize(a,a)};rat.sin=function(a,b){if(0===b[1])return rat.copy(a,rat.ZERO);rat.scalar_multiply(a,b,2);var c=rat.create();rat.pow(c,b,2);rat.add(c,c,rat.ONE);rat.divide(a,a,c);return a};
