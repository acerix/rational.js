...
rat.fromDecimal_copy=function(a,b){b=parseFloat(b);if(0===b)return rat.copy(a,rat.ZERO);if(1===b)return rat.copy(a,rat.ONE);if(Infinity===b)return rat.copy(a,rat.INFINITY);if(isNaN(b))return rat.copy(a,rat.INFINULL);if(0===b%1)return rat.fromInteger_copy(a,b);if(Math.abs(1/b%1)<rat.EPSILON)return rat.fromIntegerInverse_copy(a,Math.round(1/b));a[0]=1;a[1]=1;for(var c=[1,0,0,1],e=b,d,g=NaN,f=rat.MAX_LOOPS;f--;){integer_part=Math.floor(e);a[0]=integer_part*c[0]+c[2];a[1]=integer_part*c[1]+c[3];d=rat.toDecimal(a);
if(d===b||d===g)return a;g=d;e=1/(e-integer_part);c[2]=c[0];c[3]=c[1];c[0]=a[0];c[1]=a[1]}limit||console.log("rat.fromDecimal("+b+") reached limit at "+rat.str(a));return a};rat.fromRandom=function(a){a[0]=0xfffffffffffff*Math.random()<<0;a[1]=Math.abs(0xfffffffffffff*Math.random()<<0);return rat.normalize(a,a)};rat.sin=function(a,b){if(0===b[1])return rat.copy(a,rat.ZERO);rat.scalar_multiply(a,b,2);var c=rat.create();rat.pow(c,b,2);rat.add(c,c,rat.ONE);rat.divide(a,a,c);return a};
...
rat.traceSternBrocot=function(a){var b="";if(rat.equals(a,rat.ZERO)||rat.equals(a,rat.ONE)||rat.equals(a,rat.INFINITY)||rat.equals(a,rat.INFINULL))return b;if(rat.equals(a,rat.ZERO))return rat.copy(out,rat.ZERO);if(rat.equals(a,rat.ONE))return rat.copy(out,rat.ONE);if(rat.equals(a,rat.INFINITY))return rat.copy(out,rat.INFINITY);if(rat.equals(a,rat.INFINULL))return rat.copy(out,rat.INFINULL);var c=rat.isNegative(a);c&&(a[0]=-a[0]);for(var e=rat.clone(rat.ONE),d=[1,0,0,1],g=0,f=0,h=rat.MAX_LOOPS;!rat.approximates(a,
